...
wait_and_listen(){
  ...
  if ((init_load_status == 1));then
    ...
    echo "[WARN] latest init_load_status is $init_load_status, start to check latest load_status again."
  fi
  # wait and listen
  local i=1
  while ((i< 20))
...
check_count(){
  ...
  export partitionDt=$inner_dt
  export db=$inner_db
  export tbl=$inner_preload_tbl

  # count source-data
  export SPARK_HOME=/home/vipshop/platform/spark-3.0.1-SNAPSHOT-bin-2022-02-11-18-27-04-branch-3.0.1-fix-bucket-info
  ...
  import org.apache.spark.sql.execution.datasources.AlluxioFileSystem
  ...
  import scala.collection.JavaConversions._
      ...
      //updata meta info start
      val DORIS_DATA_LOAD="dorisLoaded"
      val DORIS_DATA_LOAD_TIMESTAMP="dorisTimestamp"

      val dt= sys.env("partitionDt").toString
      val db= sys.env("db").toString
      val tbl= sys.env("tbl").toString
      val partitionNum= sys.env("num_of_partitions").toInt

      val catalog = spark.sessionState.catalog
      val externalCatalog = catalog.externalCatalog
      val tableMeta = externalCatalog.getTable(db, tbl)

      if(isEqual){
        println("[INFO]:start update meta info")
      	if(partitionNum==0){
      		val timestamp=System.currentTimeMillis().toString
      		sql(s"alter table ${db}.${tbl} set tblproperties('${DORIS_DATA_LOAD}'='true')")
      		sql(s"alter table ${db}.${tbl} set tblproperties('${DORIS_DATA_LOAD_TIMESTAMP}'='${timestamp}')")
      		println(s"[INFO]:meta of ${db}.${tbl} has been updated")
      	}else{
      		val partitionMeta = externalCatalog.listPartitions(db, tbl)
      		val partitionSpec = partitionMeta.filter(p => p.spec.get("dt").get.equals(dt))(0)

      		val mountParts = new mutable.ListBuffer[CatalogTablePartition]()
      		val (tableIdentifier, partitionColumnNames) = (tableMeta.identifier, tableMeta.partitionColumnNames)
      		val part = catalog.getPartition(tableIdentifier, partitionSpec.spec)
      		mountParts += part.copy(parameters = part.parameters ++ Map(DORIS_DATA_LOAD -> "true")++ Map(DORIS_DATA_LOAD_TIMESTAMP -> System.currentTimeMillis().toString))

      		catalog.externalCatalog.alterPartitions(db, tbl, mountParts)
      		println(s"[INFO]:meta of ${db}.${tbl}--dt=${dt} has been updated")
      	}
      }
      //updata meta info end
